<x-layout>
    ...
    <div class="relative overflow-x-auto shadow-md rounded-lg border border-gray-200 py-2 px-2 bg-white">
        ...
        <form class="mx-auto" action="{{ route('visit.store') }}" method="post">
            @csrf
            <div class="px-6 py-2">
                <!-- Customer Info -->
                <div class="mb-2">
                    <label for="visit_date" class="mb-2 text-sm font-medium text-gray-700">Tanggal Kunjungan</label>
                    <input type="date" id="visit_date" name="visit_date" value="" autocomplete="off" class="bg-gray-50 border border-gray-300 text-gray-700 rounded-lg w-full p-2.5 text-sm focus:ring-indigo-500 focus:border-indigo-500"/>
                    @error('visit_date')
                ...
                <div class="mb-2">
                    ...
                    <textarea type="text" name="note" autocomplete="off" value="{{ old('note') }}" class="bg-gray-50 border border-gray-300 text-gray-700 rounded-lg w-full p-2.5 text-sm focus:ring-indigo-500 focus:border-indigo-500"></textarea>
                    @error('note')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                            ...
                            <div x-show="openSection" x-collapse class="space-y-4 mt-4">
                                <template x-for="(person,index) in persons" :key="index">
                                    <div class="space-y-3 border rounded-lg p-3">
                                        <select x-model="person.ocrd_person_id" :name="'persons['+index+'][ocrd_person_id]'" class="bg-gray-200 border border-gray-300 rounded-lg w-full p-2.5">
                                            <option value="">Pilih Penanggung Jawab</option>
                                            <template x-for="item in availablePersons" :key="item.id">
                                                <option :value="item.id" x-text="item.name + ' - ' + item.position"></option>
                                            </template>
                                        </select>
                                        <div class="flex justify-end">
                                            <button type="button" @click="persons.splice(index,1)" x-show="persons.length > 1" class="px-3 py-2 bg-gray-500 text-white rounded-lg">
