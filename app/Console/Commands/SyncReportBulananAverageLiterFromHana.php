...
use App\Models\Report\ReportBulananAverageLiter;
use App\Models\SyncLog;

class SyncReportBulananAverageLiterFromHana extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:reportBulananAverageLiter 
     ...
     * @var string
     */
    protected $description = 'Sync report bulanan average liter data from HANA to local DB';
        ...
        try {
            ...
            $hanaData = DB::connection('hana')->select(
                "SELECT 
                    ...
                    CAST(\"LITER\" AS NVARCHAR(50)) AS \"LITER\",
                ...
                FROM LVKKJ_REP_BULANANAVERAGELITER ('{$startDate}', '{$endDate}')"
            );

            // Kosongkan tabel
            ReportBulananAverageLiter::truncate();

            // Insert data Dashboard
            foreach ($hanaData as $row) {
                ReportBulananAverageLiter::create([
                    'MAINKEY' => $row->MAINKEY,
                    'ROWNUM' => $row->ROWNUM,
            ...
            $this->info("Total data Report Bulanan Average (Laravel): " . ReportBulananAverageLiter::count());
