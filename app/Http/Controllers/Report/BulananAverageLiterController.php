...
class BulananAverageLiterController extends Controller
{
    public function index(Request $request) 
    {
        $report = Report::where('slug', 'bulanan-dan-average-liter')->first();
        ...
        // 🔹 Base query
        $query = DB::table('report_bulanan_average_liter');
        ...
        $segments = DB::table('report_bulanan_average_liter')
        ...
        $periode = ReportBulananAverageLiter::select('tahun', 'bulan')
        ...
        $lastSync = SyncLog::where('name', 'report.bulanan-dan-average-liter')
            ->orderByDesc('last_sync')
            ->first();

        // 🔹 Jika tidak ada data
        if ($customers->isEmpty()) {
            return view('reports.bulanan_dan_average_liter', [
